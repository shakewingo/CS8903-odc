...
_current_model = None       # (exp_id, model, spatial_scale)
_initialized = False
...
def load_models():
    ...
    """
    _init_once()
    for exp_id, (run_id, _) in EXPERIMENTS.items():
...
def _get_model(experiment: str):
    ...
    if _current_model and _current_model[0] == experiment:
        return _current_model[1], _current_model[2]
    ...
    run_id, spatial_scale = EXPERIMENTS[experiment]
    ...
    _current_model = (experiment, model, spatial_scale)
    logger.info(f"Loaded model for {experiment} (run {run_id})")
    return model, spatial_scale
...
def _run_cached(lat_r: float, lng_r: float, experiment: str):
    ...
    model, spatial_scale = _get_model(experiment)
