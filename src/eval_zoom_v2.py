...
Usage:
    ...
    python src/eval_zoom_v2.py

    # Mixed methods via CLI override
    python src/eval_zoom_v2.py --experiments \\
...
def parse_args():
    ...
    p.add_argument("--output", type=str,
                   default=str(project_root / "report/final_paper/figures/zoom_comparison_v2.png"))
...
def main():
    args = parse_args()
    import src.train_v2 as train_module

    # Inference-time logger: keep quiet, write to a dedicated file.
    train_module.logger = get_logger(
        "eval_zoom_v2", stream=False, level="WARNING",
        log_file=str(Path(log_dir, "eval_zoom_v2.log")),
    )
